  ...
  return (
    <div className={styles.user}>
      ...
      <div className={styles.user__footer}>
        <div className={styles.user__status}>{user.status !== null ? user.status : "default status"}</div>
        {user.isFollowed ? (
          <button type="button" className={styles.follow__btn} onClick={() => unfollow(user.id)}>
           Unfollow
          </button>
        ) : (
          <button className={styles.follow__btn} onClick={() => follow(user.id)}>
            Follow
          </button>
        )}
      </div>
